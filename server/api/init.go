...
import (
	...
	_ "github.com/zhquiz/go-zhquiz/docs" // Generated Swagger Docs
	...
	return func(c *gin.Context) {
		...
		userName, e := func() (string, error) {
			...
			if strings.HasPrefix(authorization, "Bearer ") {
				...
				return token["identifier"].(string), nil
			}
		...
		r := resource.DB.Where("identifier = ?", userName).First(&dbUser)
		...
		if errors.Is(r.Error, gorm.ErrRecordNotFound) {
			...
			if rCreate := resource.DB.Create(&dbUser); rCreate.Error != nil {
				panic(rCreate.Error)
			}
		} else if r.Error != nil {
			panic(r.Error)
