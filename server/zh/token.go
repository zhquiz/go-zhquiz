...
type Token struct {
	...
	Tag      []Tag   `gorm:"many2many:token_tag"`
}
...
// TokenTag is joint Table for Token-Tag
// @internal
type TokenTag struct {
	Entry int   `gorm:"primaryKey"`
	TagID int   `gorm:"primaryKey"`
	Token Token `gorm:"foreignKey:Entry"`
	Tag   Tag   `gorm:"foreignKey:Tag"`
}
